  ...
  <div class="versions-container">
    ...
    <div class="controls">
      ...
      <input 
        ...
        type="color" 
        title="选择均线颜色"
        style="width: 40px; height: 40px; border: 1px solid #ddd; border-radius: 4px; cursor: pointer;"
      />
...
import { ref, onMounted, onUnmounted, nextTick, watch } from 'vue'  // 添加 nextTick 和 watch 导入
...
// 均线颜色
const maColor = ref<string>('#ff9f43') // 默认颜色
...
// 监听均线颜色变化，重新渲染图表
watch(maColor, () => {
  if (analysisResult.value && chartContainer.value) {
    // 重新渲染图表以应用新的均线颜色
    const rawData = generateLotteryData()
    if (selectedNumbers.value && selectedNumbers.value.length > 0) {
      const result = analyzeMultipleNumbers(rawData, selectedNumbers.value)
      renderNumberTrend(result.data)
    } else {
      const result = analyzeSingleNumber(rawData, targetNumber.value)
      renderNumberTrend(result.data)
...
const resetAnalysis = () => {
  ...
  maColor.value = '#ff9f43' // 重置均线颜色为默认值
