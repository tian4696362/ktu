<template>
  ...
  <div class="versions-container">
    ...
    <div class="controls">
      ...
      <button @click="analyzeNumber">单个分析</button>
      <button @click="resetAnalysis">重置</button>
    </div>
    
    <div v-if="analysisResult" class="analysis-info">
      <div v-for="result in analysisResults" :key="result.number">
        <p><strong>号码 {{ result.number }} 理论概率: </strong>{{ result.theoreticalProbability }}</p>
        <p><strong>当前指数: </strong>{{ result.currentIndex }}</p>
        <p><strong>中奖次数: </strong>{{ result.hitCount }}</p>
        <p><strong>中奖频率: </strong>{{ result.hitRate }}%</p>
      </div>
    </div>
...
const analyzeSelectedNumbers = () => {
  ...
  validNumbers.forEach(number => {
    const result = analyzeSingleNumber(rawData, number)
  ...
  // 渲染多号码趋势图
  renderMultipleNumberTrend(validNumbers, rawData)
}
...
const analyzeNumber = () => {
  ...
  analysisResults.value = [{
    ...
    theoreticalProbability: result.theoreticalProbability,
    currentIndex: result.currentIndex,
    hitCount: result.hitCount,
    hitRate: result.hitRate.toFixed(2)
  }]
  
  renderNumberTrend(result.data, targetNumber.value)
}
...
// 绘制单个号码趋势图
const renderNumberTrend = (data: any[], number: number) => {
  ...
  // 计算移动平均线
  const maData = calculateMovingAverage(klineData, 5);
  
  const option: echarts.EChartsOption = {
    title: {
      text: `号码 ${number} 中奖趋势分析`,
      left: 'center',
      textStyle: {
        fontSize: 16
      }
    ...
    series: [
      {
        ...
        data: klineData,
        itemStyle: {
          color: '#ef5350',      // 中奖（上涨）为红色
          color0: '#26a69a',     // 未中奖（下跌）为绿色
          borderColor: '#ef5350',
          borderColor0: '#26a69a'
        }
      },
...
// 绘制多个号码趋势图
const renderMultipleNumberTrend = (numbers: number[], rawData: any[]) => {
  if (!chartContainer.value) return;
  ...
  const seriesData = []
  const legendData = []
  const allPeriods = rawData.map(item => item.period)
  
  numbers.forEach(number => {
    const result = analyzeSingleNumber(rawData, number)
    const klineData = generateKlineData(result.data)
    ...
    seriesData.push({
      ...
      data: klineData,
      itemStyle: {
        color: getRandomColor(),      // 随机颜色表示中奖（上涨）
        color0: getRandomColor(),    // 随机颜色表示未中奖（下跌）
        borderColor: getRandomColor(),
        borderColor0: getRandomColor()
      }
    })
    
    legendData.push(`号码${number}`)
  })
  
  const option: echarts.EChartsOption = {
    title: {
      text: `号码 ${numbers.join(', ')} 中奖趋势分析`,
      left: 'center',
      textStyle: {
        fontSize: 16
      }
    ...
    tooltip: {
      ...
      formatter: function(params: any) {
        const dataIndex = params[0].dataIndex;
        const periodInfo = rawData[dataIndex];
        
        let result = `期号: ${periodInfo.period}<br/>日期: ${periodInfo.date}<br/>`;
        
        numbers.forEach(number => {
          const isHit = periodInfo.numbers.includes(number);
          result += `号码${number}中奖: ${isHit ? '是' : '否'}<br/>`;
        });
...
const getRandomColor = () => {
  ...
  return colors[Math.round(Math.random() * (colors.length - 1))] || '#c23531';
}
